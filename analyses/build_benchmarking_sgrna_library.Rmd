...
```{r}
croatan <- read.table("../data/CRISPR_libraries/HG19_Library_For_Build.with_gene.txt",
  ...
  rowwise() %>%
  mutate(sequence = substr(context_sequence,1,24), PAM = substr(context_sequence,25,27)) %>%
  ungroup()
