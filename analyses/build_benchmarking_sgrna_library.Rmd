...
```{r}
# Handle the missing genes.
missing.genes <- croatan$gene_symbol[is.na(croatan$sequence)]

# Unspliced gene (ens99) and exon (ens75) narrows down the number of missing genes.
miss_gene.grnas <- read_sam("../data/ens99-croatan-missing-genes-unspl-gene.sam") %>%
  rbind(read_sam("../data/ens75-croatan-missing-genes-exon.sam")) %>%
  filter(!duplicated(V10)) %>%
  rename(sam_flag = V2, gene = V3, bowtie2_matches = V6, sequence = V10) %>%
  arrange(gene) %>%
  rowwise() %>%
  mutate(in_prior_set = sequence %in% croatan$sequence) %>%
  ungroup() %>%
  mutate(other_gene = croatan$gene_symbol.fgc[match(sequence,croatan$sequence)],
         gene_symbol.fgc = genes$gene[match(gene,genes$gene_symbol)])

missing.genes.final <- genes$Gene[match(setdiff(missing.genes,miss_gene.grnas$gene),genes$gene_symbol)]

cat("Multi-mapping gRNAs:")
as.data.frame(table(miss_gene.grnas$gene, miss_gene.grnas$other_gene)) %>%
  filter(Freq>0)
