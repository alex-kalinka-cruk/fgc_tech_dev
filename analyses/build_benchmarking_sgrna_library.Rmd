...
read_sam <- function(sam_out){
  ...
  aln <- read.delim(sam_out, header=F, comment.char = "@", sep="\t") %>%
    select(V3, V6, V10) %>%
...
missing_gene.seqs <- build_missing_gene_bowtie_ref(missing.genes, "missing_croatan.fasta",
                                                   "croatan", ensembl.hg37)
