...
# Sanity check Croatan PAM sequences.
cat("All Croatan PAM sequences end in 'GG':")
all(unlist(sapply(croatan$PAM, function(x) gsub("^.(..)$","\\1",x))) == "GG")

# Additional 5' and 3' sequence for common vector.
common_seq.vector <- read.csv("../data/oligo-design-template-CRISPR-benchmarking.csv", stringsAsFactors = F)
...
write.csv(genes %>%
            select(-Cluster), 
          file = "../data/benchmarking_input_genes.csv", quote=F, row.names = F)
```
...
pct_shared_grnas <- function(data){
  data %<>%
    mutate(Library_pair = paste(Library_1,Library_2,sep="."))
  libs <- unique(data$Library_1)
  ret <- NULL
  for(lib in libs){
    td <- data %>%
      filter(grepl(lib,Library_pair)) %>%
      mutate(Lib = lib, gRNA_percent = 100*gRNA_count/sum(gRNA_count)) %>%
      rowwise() %>%
      mutate(Library = ifelse(Library_pair!=paste(lib,lib,sep="."),
                          setdiff(unlist(strsplit(Library_pair,"\\.")),unique(Lib)),
                          lib)) %>%
      ungroup()
    ret <- rbind(ret, td)
  }
...
make_library_dist <- function(data){
  data %<>%
    spread(Library_2, gRNA_count)
  rn <- data$Library_1
  cn <- colnames(data)[2:ncol(data)]
  data <- as.matrix(data[,2:ncol(data)])
  rownames(data) <- rn
  colnames(data) <- cn
  data <- 1-data/rowSums(data)
  return(data)
}
```

```{r}
cat("Total number of gRNAs:")
nrow(bm_library)
...
cat("Duplicate gRNA count:")
sum(duplicated(bm_library$sequence))
...
bm_lib.pct <- pct_shared_grnas(bm_lib.hmap %>%
                                 filter(!between(row_number(),22,42)))

dist.bm_lib <- make_library_dist(bm_lib.hmap)
hcl.bm_lib <- hclust(as.dist(dist.bm_lib), method="ward.D2")

num_grnas_per_gene <- bm_library %>%
  group_by(gene) %>%
  summarise(num_grnas = n()) %>%
  ungroup()

# Plots.
ggplot(num_grnas_per_gene, aes(num_grnas)) +
  geom_bar() +
  ggtitle("Number of gRNAs per gene")
...
ggplot(bm_lib.pct, aes(Library, gRNA_percent)) +
  geom_bar(stat="identity") +
  facet_grid(~Lib, space="free_x", ) +
  theme(axis.text.x = element_text(angle = -30, hjust=0, size=7)) +
  ggtitle("Percent of shared gRNAs across libraries")

ggdendrogram(hcl.bm_lib, size=5,theme_dendro=F) + 
  theme(axis.text.x = element_text(angle=-30,hjust=0,size=10)) + 
  xlab("") + 
  ylab("Distance (Ward D^2)") + 
  ggtitle("gRNA Library dendrogram")
...
yusa.ctrl <- yusa.v3 %>%
  filter(grepl("Control",gRNA_ID)) %>%
  head(100) %>%
  rename(gene = Gene, grna_id = gRNA_ID, sequence = Guide_sequence) %>%
  mutate(library = "Control", strand = NA, target_exon = NA) %>%
  select(library, gene, grna_id, strand, target_exon, sequence)

bm_library %<>%
  rbind(yusa.ctrl) %>%
  # Add common vector 5' and 3' vector sequence.
  mutate(sequence_to_order = paste(common_seq.vector$upstream.sequence.5prime, sequence,
                                   common_seq.vector$downstream.sequence.3prime, sep="")) %>%
  arrange(gene, library)
...
```{r}
write.csv(bm_library, file = "../data/benchmark_library_FGC.csv", 
          row.names = F, quote=F)
```
